...
void racer::RaceKart::UpdateLogic(float deltaTime, std::map<pengine::Input, long>* actions)
{
...
void racer::RaceKart::Render(pengine::Renderer* renderer)
{
	if (model != NULL)
	{
...
void racer::RaceKart::SetObject3D(pengine::Object3D* p_xModel)
{
	model = p_xModel;
}

void racer::RaceKart::SetControllable(bool p_controllable)
{
	controllable = p_controllable;
}

void racer::RaceKart::SetHorsePower(float p_horsePower)
{
	horsePower = p_horsePower;
}

float racer::RaceKart::GetHorsePower()
{
	return horsePower;
}

void racer::RaceKart::Brake(float percentage)
{
	// 5.0f could be replaced by braking power
	ApplyFriction(5.0f * percentage);
}

void racer::RaceKart::Throttle(float percentage)
{
	if (!collides)
	{
...
void racer::RaceKart::Steer(float percentage)
{
	if (!collides)
	{
		pengine::Vector3 vector;
...
void racer::RaceKart::OnCollide(pengine::COLLISIONEFFECT* effect)
{
	if (!effect->isStatic)
	{
		collides = true;
...
void racer::RaceKart::InitCollisionBox()
{
	// Get bounds of model
...
std::string racer::RaceKart::GetType()
{
	return "racer::RaceCart";
}

pengine::Vector3* racer::RaceKart::GetCollisionForceVector()
{
	return &movementVector;
}

float racer::RaceKart::GetCollisionMass()
{
	return mass;
}

float racer::RaceKart::GetRadius(){
	float radius = collisionBox.depth * scale.z;
	return radius;
}

void racer::RaceKart::CacheToRenderer(pengine::Renderer* renderer)
{
	model->CacheToRenderer(renderer);
}

void racer::RaceKart::AddCheckPoint(TrackBlock* checkPoint)
{
	checkPoints.push_back(checkPoint);
}

std::list<racer::TrackBlock*>* racer::RaceKart::GetCheckPoints()
{
	return &checkPoints;
}

bool racer::RaceKart::IsOnTrack()
{
	return isOnTrack;
}

void racer::RaceKart::SetLastCheckPoint(TrackBlock* checkPoint)
{
	lastCheckPoint = checkPoint;
}

void racer::RaceKart::SetTrackHeight(float height)
{
	trackHeight = height;
}